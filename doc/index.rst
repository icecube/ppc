...
New sensor implementation (NEXTGEN): simulation of multi-PMT sensors in ppc
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

At the moment, only the plane-wave approximation is implemented. This means that the hits are registered and assigned to PMTs only based on the direction n of the photon, and do not depend on the landing coordinates on the surface of the optical module. The OM shape is approximated with a spheroid that should roughly match in the dimensions of and occupy similar volume to the actual OM. The area presented by the simulated spheroid to the photon will depend on the photon direction n: A(n).

At the moment, the angular response of each PMT within the OM is paramterized with the same function f(beta, (m*n)) of the direction the PMT is pointing at (m) and a shape parameter beta. Function f was designed to describe the visible area of a slice of a sphere with a height=(1-beta)*radius from different directions and was found to describe the lab-measured angular sensitivity of a nominal IceCube DOM well with beta=0.49. In the extremes, beta=1 describes a flat PMT and beta=0 describes projected area of a half-sphere. We fitted a value beta=0.97 and PMT axis elevation angles offset from nominal by 8 and 15 degrees for first and second-ring PMTs of a LOM-16, respectively, to the Geant simulation of the OM. In the future we might improve this model by a description with two terms per PMT (each with their own beta and elevation angles).

The effective area table of the sensor (vs. wavelength) is given averaged over all directions: EA(w). Before simulation, ppc calculates two factors. One is the maximum ratio over all directions (sampled from a set of icosahedral-based roughly uniform on a sphere 642 directions) of sum1=maximum[Sum_k{f(beta, (m_k*n))/A(n)}] (k is pmts). Second is the average over all directions of ave2=average[Sum_k{f(beta, (m_k*n))}]. Now, the efficiency of spheroid approximating our OM (to be simulated at this stage) is calculated as EA*sum1/ave2.

After photon propagation with spheroid OMs with efficiency calculated above, we assign PMTs. We have so far simulated the spheroid OM that has the same (maximum) efficiency for each direction. This maximum efficiency is only achieved for some directions, for others the efficiency will be smaller. At this point we can apply the ratio of [Sum_k{f(beta, (m_k*n))}/A(n)]/sum1 for each specific photon direction to apply the necessary reduction of efficiency from maximum, and then choose the pmt by sampling from the Sum_k{f(beta, (m_k*n))}.

The method described above can be used for the simulation of a regular IceCube DOM. However, we have traditionally used a simplified version of this, where the angular sensitivity of a DOM is given by a fully tabulated function of the photon angle with respect to the PMT axis, and the OM sensitive area is given for the direction the PMT of the OM is facing. We also have an implementation (named "HD" angular sensitivity model) where instead of the planewave approximation depending on the photon direction, the photon landing coordinates on the surface of the DOM are used instead (accepting photons which land on a portion of the lower hemisphere where the sensitive area of the PMT is located). We have yet to extend the multi-PMT simulation in a similar way.
